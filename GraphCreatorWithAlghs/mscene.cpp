...
MScene::MScene(QObject *par)
	:QGraphicsScene(par),show_edge_values(false),graph_oriented(true),
	  node_diametr(60), arrow_size(15),node_shape(Node::Circle)
{
...
int MScene::nodeShape() const {return node_shape;}

int MScene::diametr() const{return node_diametr;}

QList<Node*> * MScene::nodesList(){return &all_nodes;}

MGraphView * MScene::getMGraph(){return parent;}
...
{
	bool ok;
	int size = QInputDialog::getInt(getMGraph(),"Изменить размер","Размер",s,30,120,1,&ok);
	if(!ok || !size)
		return;
	foreach (QGraphicsItem * it, selectedItems()) {
		if(it->type() == Node::Type)
			((Node*)it)->setDiametr(size);
	}
	//clearSelection();
}
...
void MScene::setDiametr(int d){	node_diametr = d;}

void MScene::setArrowSize(int d){	arrow_size = d;}

bool MScene::isOriented()const {	return graph_oriented;}
...
//если уже соединены - остановить
	...
	if(graph_oriented) //если граф ориентированный
	{
...
void MScene::setShowEdge(bool a){show_edge_values = a; update();}

bool MScene::shouldShowValues()const {return show_edge_values;}

//установить цвет выделенных вершин
void MScene::setColorOfSelectedNodes(QColor c)
{
	QColor color = QColorDialog::getColor(c,parent,QString("Цвет вершины"));
	if(!color.isValid())
		return;
