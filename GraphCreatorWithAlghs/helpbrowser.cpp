...
#include "mscene.h"
#include "edge.h"
InformationDialog::InformationDialog(const QString& strPath,
...
InformationDialog::InformationDialog(MScene * scene, QWidget *par)
{
	...
	QVBoxLayout * main_lay = new QVBoxLayout;
		...
		QGroupBox * diam = new QGroupBox("Диаметр вершин");
		...
		diam->setLayout(diam_lay);
			...
			diam_edit->setNum(scene->diametr());
			diam_slider->setRange(30,120);
			diam_slider->setValue(scene->diametr());
		...
		QGroupBox * ori = new QGroupBox("Ориентированный граф");
		QHBoxLayout* ori_lay = new QHBoxLayout;
		ori->setLayout(ori_lay);
			QRadioButton * yes = new QRadioButton("Да");
			QRadioButton * no = new QRadioButton("Нет");
			if(oriented)
				yes->setChecked(true);
			else
				no->setChecked(true);
		...
		QGroupBox * arrow_size = new QGroupBox("Размер стрелки");
		...
		QHBoxLayout * but = new QHBoxLayout;
			QPushButton * ok_but = new QPushButton("Ок");
			QPushButton * appl_but = new QPushButton("Применить");
...
InformationDialog::InformationDialog(QWidget *par)
	:QDialog(par){}
...
{
	scene_for_set->setDiametr(new_diam);
