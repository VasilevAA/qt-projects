...
void Edge::popupAnim()
{
	...
	//соединение завершения таймера и удаление всех созданных объектов
		...
		if(it->boundingRect().width() != 102) //изменить картинку транспортного средства
		{
			QLineF line(sourcePoint, destPoint);

			QPixmap p(":/cars/" +QString::number(line.length()>=1000
											 ? 16
											 :line.length()>=500
											   ? qrand()%4 + 12
											   :qrand()%11+1)
...
void Edge::animate()
{
	...
	QPixmap p(":/cars/" +QString::number(line.length()>=1000 //выбор случайной  картинки
										 ? qrand()%2 + 16
										 :line.length()>=500
										   ? qrand()%4 + 12
										   :qrand()%11+1)
	...
	if(p.width()==102) //установка винта
	{
		QPixmap pron(":/cars/pron.png");
		dop_it->setPixmap(pron);
		dop_it->setOffset(-56.5,-38.5);;
		dop_it->setTransformationMode(Qt::SmoothTransformation);
		dop_it->setPos(17,0);
		dop_it->setZValue(0.7);
		dop_it->setParentItem(it);

		dop_posAnim->setItem(dop_it);
		dop_posAnim->setRotationAt(0,0);
		dop_posAnim->setRotationAt(1.0,360);

		dop_timer->setCurveShape(QTimeLine::LinearCurve);
		dop_timer->start();
...
void Edge::paint(QPainter *painter, const QStyleOptionGraphicsItem *, QWidget *)
{
	...
	painter->setPen(QPen((it && (it->boundingRect().size().width()==102
								 || it->boundingRect().size().height()==80))? Qt::transparent:Qt::lightGray, 22, style, Qt::RoundCap, Qt::RoundJoin));
	painter->drawPath(a);
	...
	{ //отрисовка стрелки посередине

	QLineF l1(a.pointAtPercent(0.3),a.pointAtPercent(0.32));
	double ar_ang1 = ::acos(l1.dx() / l1.length());
	if(l1.dy() >= 0)
		ar_ang1 = TwoPi - ar_ang1;

	painter->setPen(Qt::SolidLine);
	painter->setBrush(main_color);

	QPointF destArrowP11 = a.pointAtPercent(0.31) + QPointF(sin(ar_ang1 - Pi / 2.7) * arrowSize,
											  cos(ar_ang1 - Pi / 2.7) * arrowSize);
	QPointF destArrowP22 = a.pointAtPercent(0.31) + QPointF(sin(ar_ang1 - Pi + Pi / 2.7) * arrowSize,
											  cos(ar_ang1 - Pi + Pi / 2.7) * arrowSize);

	painter->drawPolygon(QPolygonF() << a.pointAtPercent(0.31) << destArrowP11 << destArrowP22);

	}

	{ //отрисовкаа конечной стрелки

	QLineF l(c,destPoint);
	double ar_ang = ::acos(l.dx() / l.length());
	if(l.dy() >= 0)
		ar_ang = TwoPi - ar_ang;
