...
MGraphView * MScene::getMGraph() const {return parent;}
...
//если выделено 2 элемента
	if(selectedItems().size() == 2 && QApplication::keyboardModifiers() == Qt::ShiftModifier )
	{
		...
		if (Node::is_two_nodes_connected(first,(Node*)second))
		{
...
{
	foreach(QGraphicsItem* node,deleted_it) //очистить удаленные объекты
		delete node;
	...
	for (int i = 0; i < nodes.size(); i++)
	{
		...
		for(int k = 0; k < child_of_nodes[i].size(); k++)
		{
			...
			if (a && b) //елси найдены
			{
				if(a == b) //совпадают - создается петля
					a->setLoop();

				if (!Node::is_two_nodes_connected(a,b)) //не соединены - добавить соединение
					addEdge(a, b);
			}
			else
				continue;
