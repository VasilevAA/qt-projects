...
void Node::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *)
{
	...
	if(childrenList.contains(this)) //отрисовка петли
	{
		painter->setPen(QPen(Qt::black,2, Qt::SolidLine, Qt::RoundCap, Qt::RoundJoin));
		painter->drawEllipse(-diametr/2-10,-diametr/2-10,diametr/2,diametr/2);
		painter->setBrush(Qt::black);
		painter->drawPolygon(QPolygonF() << QPointF(-34,-20) << QPointF(-28,-10)<< QPointF(-38,-10));
	}
...
void Node::contextMenuEvent(QGraphicsSceneContextMenuEvent *event)
{
	...
	menu.addSeparator();
		...
		QAction *copy_action = menu.addAction("Копировать");
	...
	else if(selected_action == rename_action)
	{
		...
		removeLoop();
	else if(selected_action == copy_action)
		scene->copySelectedNodes(this);
	setSelected(false);
